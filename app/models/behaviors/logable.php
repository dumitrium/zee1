...
class LogableBehavior extends ModelBehavior 
{
  ...
  public $settings = array();
	...
	//	pr($this->settings[$Model->alias]);break;
	...
	//	pr($keys);break;
		...
		//pr($keys);
	//	pr($Model->data[$Model->alias]['ingredient_id']);
	if (sizeof($diff) == 0 && empty($Model->logableAction)) {
		...
		if (isset($Model->data[$Model->alias]['ingredient_id']))
			{
				$logData['Log'][$this->settings[$Model->alias]['foreignKey']]=$Model->data[$Model->alias]['ingredient_id'];
			}
			elseif (isset($Model->data[$Model->alias]['product_id'])){
				$logData['Log'][$this->settings[$Model->alias]['foreignKey']]=$Model->data[$Model->alias]['product_id'];
					}else{$logData['Log'][$this->settings[$Model->alias]['foreignKey']] = $id;}
		
		
				//	$logData['Log'][$this->settings[$Model->alias]['tip']]=$this->settings[$Model->alias]['tip'];
    ...
    //	}
    	...
    	$this->_saveLog($Model, $logData, $keys);
    }
    ...
    /*	if ($title !== NULL) {
    	...
    	if(isset($logData['Log']['ingredient_id'])){
    	$logData['Log']['title']=$logData['Log']['ingredient_id'];
    	$logData['Log']['tip']='ingredient';}	elseif
    	(isset($logData['Log']['product_id'])){
    	$logData['Log']['title']=$logData['Log']['product_id'];
    	$logData['Log']['tip']='product';}else
	    	{
    	$logData['Log']['title']=$logData['Log']['model_id'];
    	if($Model->name=='Ingredient'){
    	$logData['Log']['tip']='ingredient';}else{$logData['Log']['tip']='product';}
    	...
    	if(isset($this->Log->_schema['tip']) && isset($Model->tip))
	    	{$logData['Log']['tip']=$Model->tip;}
	    	
	    if(isset($this->Log->_schema['tip_id']) && isset($Model->tip_d))
	    	{$logData['Log']['tip_id']=$Model->tip_id;}
