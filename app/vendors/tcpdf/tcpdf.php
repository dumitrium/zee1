...
class TCPDF {
	 ...
	 * @protected
	 */
	var $cell_padding = array('T' => 0, 'R' => 0, 'B' => 0, 'L' => 0);
	 ...
	 * @protected
	 */
	var $cell_margin = array('T' => 0, 'R' => 0, 'B' => 0, 'L' => 0);
	 ...
	 * @protected
	 */
	var $header_xobj_autoreset = false;
	 ...
	 * @protected
	 */
	var $enc_padding = "\x28\xBF\x4E\x5E\x4E\x75\x8A\x41\x64\x00\x4E\x56\xFF\xFA\x01\x08\x2E\x2E\x00\xB6\xD0\x68\x3E\x80\x2F\x0C\xA9\xFE\x64\x53\x69\x7A";
	 ...
	 * @since 4.6.005 (2009-04-24)
	 */
	var $signature_max_length = 11742;
	 ...
	 */
	var $signature_appearance = array('page' => 1, 'rect' => '0 0 0 0');
	 ...
	 * @since 4.8.000 (2009-09-07)
	 */
	var $default_form_prop = array('lineWidth'=>1, 'borderStyle'=>'solid', 'fillColor'=>array(255, 255, 255), 'strokeColor'=>array(128, 128, 128));
	 ...
	 */
	var $maxselcol = array('page' => 0, 'column' => 0);
	 ...
	 * @since 5.8.000 (2010-08-11)
	 */
	var $colxshift = array('x' => 0, 's' => array('H' => 0, 'V' => 0), 'p' => array('L' => 0, 'T' => 0, 'R' => 0, 'B' => 0));
	 ...
	 */
	var $rasterize_vector_images = false;
	 ...
	 */
	var $default_graphic_vars = array();
	 ...
	 * @since 5.0.000 (2010-05-02)
	 */
	var $svginheritprop = array('clip-rule', 'color', 'color-interpolation', 'color-interpolation-filters', 'color-profile', 'color-rendering', 'cursor', 'direction', 'fill', 'fill-opacity', 'fill-rule', 'font', 'font-family', 'font-size', 'font-size-adjust', 'font-stretch', 'font-style', 'font-variant', 'font-weight', 'glyph-orientation-horizontal', 'glyph-orientation-vertical', 'image-rendering', 'kerning', 'letter-spacing', 'marker', 'marker-end', 'marker-mid', 'marker-start', 'pointer-events', 'shape-rendering', 'stroke', 'stroke-dasharray', 'stroke-dashoffset', 'stroke-linecap', 'stroke-linejoin', 'stroke-miterlimit', 'stroke-opacity', 'stroke-width', 'text-anchor', 'text-rendering', 'visibility', 'word-spacing', 'writing-mode');
	 ...
	 */
	function getPageSizeFromFormat($format) {
	 ...
	 * @since 5.0.010 (2010-05-17)
	 */
	function setPageBoxes($page, $type, $llx, $lly, $urx, $ury, $points=false) {
	 ...
	 * @since 5.0.010 (2010-05-17)
	 */
	function swapPageBoxCoordinates($page) {
		$pageboxes = array('MediaBox', 'CropBox', 'BleedBox', 'TrimBox', 'ArtBox');
	 ...
	 */
	function setCellPaddings($left='', $top='', $right='', $bottom='') {
		if (($left !== '') AND ($left >= 0)) {
	 ...
	 * @see getCellMargins()
	 */
	function setCellMargins($left='', $top='', $right='', $bottom='') {
		if (($left !== '') AND ($left >= 0)) {
	 ...
	 * @since 5.9.000 (2010-10-03)
	 */
	function adjustCellPadding($brd=0) {
		if (empty($brd)) {
			return;
		}
		if (is_string($brd)) {
	 ...
	 * @public
	 */
	function SetDocInfoUnicode($unicode=true) {
	 ...
	 */
	function addTOCPage($orientation='', $format='', $keepmargins=false) {
	 ...
	 */
	function endPage($tocpage=false) {
	 ...
	 * @public
	 */
	function startPage($orientation='', $format='', $tocpage=false) {
		if ($tocpage) {
			$this->tocpage = true;
		}
	 ...
	 * @public
	 */
	function setHeaderTemplateAutoreset($val=true) {
	 ...
	 * @see SetDrawColor()
	 */
	function SetDrawColorArray($color, $ret=false) {
		if (is_array($color)) {
	 ...
	 * @see SetFillColor()
	 */
	function SetFillColorArray($color, $ret=false) {
		if (is_array($color)) {
			$color = array_values($color);
			$r = isset($color[0]) ? $color[0] : -1;
			$g = isset($color[1]) ? $color[1] : -1;
	 ...
	 * @see SetFillColor()
	 */
	function SetTextColorArray($color, $ret=false) {
		if (is_array($color)) {
			$color = array_values($color);
			$r = isset($color[0]) ? $color[0] : -1;
			$g = isset($color[1]) ? $color[1] : -1;
	 ...
	 * @since 1.2
	 */
	function GetStringWidth($s, $fontname='', $fontstyle='', $fontsize=0, $getarray=false) {
	 ...
	 */
	function GetArrStringWidth($sa, $fontname='', $fontstyle='', $fontsize=0, $getarray=false) {
	 ...
	 * @since 2.4.000 (2008-03-06)
	 */
	function GetCharWidth($char, $notlast=true) {
		// get raw width
	 ...
	 */
	function AddFont($family, $style='', $fontfile='', $subset='default') {
		if ($subset === 'default') {
	 ...
	 */
	function getFontDescent($font, $style='', $size=0) {
	 ...
	 * @since 4.9.003 (2010-03-30)
	 */
	function getFontAscent($font, $style='', $size=0) {
		$fontdata = $this->AddFont($font, $style);
		$fontinfo = $this->getFontBuffer($fontdata['fontkey']);
	 ...
	 * @see Cell()
	 */
	function getCellCode($w, $h=0, $txt='', $border=0, $ln=0, $align='', $fill=false, $link='', $stretch=0, $ignore_min_height=false, $calign='T', $valign='M') {
	 ...
	 */
	function getCellBorder($x, $y, $w, $h, $brd) {
	 ...
	 * @since 4.4.002 (2008-12-09)
	 */
	function getBorderMode($brd, $position='start') {
	 ...
	 * @since 4.5.011
	 */
	function getNumLines($txt, $w=0, $reseth=false, $autopadding=true, $cellpadding='', $border=0) {
		if ($txt === '') {
			// empty string
			return 1;
		}
	 ...
	 * @public
	 */
	function getStringHeight($w, $txt, $reseth=false, $autopadding=true, $cellpadding='', $border=0) {
		// adjust internal padding
		$prev_cell_padding = $this->cell_padding;
		$prev_lasth = $this->lasth;
		if (is_array($cellpadding)) {
	 ...
	 */
	function getImageFileType($imgfile, $iminfo=array()) {
		$type = '';
	 ...
	 */
	function fitBlock($w, $h, $x, $y, $fitonpage=false) {
		if ($w <= 0) {
			// set maximum width
	 ...
	 * @since 4.9.016 (2010-04-20)
	 */
	function _setGDImageTransparency($new_image, $image) {
		// transparency index
	 ...
	 * @see Image()
	 */
	function ImagePngAlpha($file, $x, $y, $wpx, $hpx, $w, $h, $type, $link, $align, $resize, $dpi, $palign, $filehash='') {
		if (empty($filehash)) {
			$filehash = md5($file);
		}
	 ...
	 * @see GetX(), GetY(), SetY(), SetXY()
	 */
	function SetY($y, $resetx=true, $rtloff=false) {
		if ($resetx) {
			//reset x
			if (!$rtloff AND $this->rtl) {
				$this->x = $this->w - $this->rMargin;
	 ...
	 * @see SetX(), SetY()
	 */
	function SetXY($x, $y, $rtloff=false) {
	 ...
	 * @since 4.8.001 (2009-09-09)
	 */
	function _putAPXObject($w=0, $h=0, $stream='') {
		$stream = trim($stream);
		$out = $this->_getobj()."\n";
	 ...
	 */
	function _dolinethroughw($x, $y, $w) {
		$linew = - $this->CurrentFont['ut'] / 1000 * $this->FontSizePt;
	 ...
	 */
	function UTF8ArrToLatin1($unicode) {
	 ...
	 * @public
	 */
	function addHtmlLink($url, $name, $fill=false, $firstline=false, $color='', $style=-1, $firstblock=false) {
	 ...
	 * @public
	 */
	function convertHTMLColorToDec($hcolor='#FFFFFF') {
		$returncolor = false;
	 ...
	 * @author Nicola Asuni
	 */
	function _fixAES256Password($password) {
	 ...
	 * @author Nicola Asuni
	 */
	function getUserPermissionCode($permissions, $mode=0) {
		$options = array(
	 ...
	 * @author Nicola Asuni
	 */
	function SetProtection($permissions=array('print', 'modify', 'copy', 'annot-forms', 'fill-forms', 'extract', 'assemble', 'print-high'), $user_pass='', $owner_pass=null, $mode=0, $pubkeys=null) {
	 ...
	 * @author Nicola Asuni
	 */
	function convertHexStringToString($bs) {
	 ...
	 * @author Nicola Asuni
	 */
	function convertStringToHexString($s) {
		$bs = '';
	 ...
	 * @author Nicola Asuni
	 */
	function getEncPermissionsString($protection) {
	 ...
	 * @since 2.1.000 (2008-01-08)
	 */
	function SetLineStyle($style, $ret=false) {
		$s = ''; // string to be returned
		if (!is_array($style)) {
			return;
		}
	 ...
	 */
	function _outCurveV($x2, $y2, $x3, $y3) {
	 ...
	 * @since 2.1.000 (2008-01-08)
	 */
	function _outCurveY($x1, $y1, $x3, $y3) {
	 ...
	 * @since 2.1.000 (2008-01-08)
	 */
	function Ellipse($x0, $y0, $rx, $ry='', $angle=0, $astart=0, $afinish=360, $style='', $line_style=array(), $fill_color=array(), $nc=2) {
	 ...
	 * @since 4.9.019 (2010-04-26)
	 */
	function _outellipticalarc($xc, $yc, $rx, $ry, $xang=0, $angs=0, $angf=360, $pie=false, $nc=2, $startpoint=true, $ccw=true, $svg=false) {
		$k = $this->k;
		if ($nc < 2) {
			$nc = 2;
		}
	 ...
	 * @since 2.1.000 (2008-01-08)
	 */
	function Circle($x0, $y0, $r, $angstr=0, $angend=360, $style='', $line_style=array(), $fill_color=array(), $nc=2) {
	 ...
	 * @public
	 */
	function PolyLine($p, $style='', $line_style=array(), $fill_color=array()) {
	 ...
	 * @since 2.1.000 (2008-01-08)
	 */
	function Polygon($p, $style='', $line_style=array(), $fill_color=array(), $closed=true) {
	 ...
	 */
	function RoundedRectXY($x, $y, $w, $h, $rx, $ry, $round_corner='1111', $style='', $border_style=array(), $fill_color=array()) {
	 ...
	 */
	function utf8StrArrRev($arr, $str='', $setbom=false, $forcertl=false) {
	 ...
	 * @since 2.1.002 (2008-02-12)
	 */
	function Bookmark($txt, $level=0, $y=-1, $page='', $style='', $color=array(0,0,0)) {
		if ($level < 0) {
			$level = 0;
		}
	 ...
	 * @since 4.8.000 (2009-09-07)
	 */
	function addJavascriptObject($script, $onload=false) {
		++$this->n;
	 ...
	 */
	function getAnnotOptFromJSProp($prop) {
	 ...
	 * @since 4.8.000 (2009-09-06)
	 */
	function setFormDefaultProp($prop=array()) {
	 ...
	 * @since 4.8.000 (2009-09-07)
	 */
	function TextField($name, $w, $h, $prop=array(), $opt=array(), $x='', $y='', $js=false) {
		if ($x === '') {
			$x = $this->x;
		}
		if ($y === '') {
	 ...
	 * @since 4.8.000 (2009-09-07)
	 */
	function RadioButton($name, $w, $prop=array(), $opt=array(), $onvalue='On', $checked=false, $x='', $y='', $js=false) {
		if ($x === '') {
			$x = $this->x;
		}
		if ($y === '') {
	 ...
	 * @since 4.8.000 (2009-09-07)
	 */
	function ListBox($name, $w, $h, $values, $prop=array(), $opt=array(), $x='', $y='', $js=false) {
		if ($x === '') {
			$x = $this->x;
		}
		if ($y === '') {
	 ...
	 * @since 4.8.000 (2009-09-07)
	 */
	function ComboBox($name, $w, $h, $values, $prop=array(), $opt=array(), $x='', $y='', $js=false) {
		if ($x === '') {
			$x = $this->x;
		}
		if ($y === '') {
	 ...
	 * @since 4.8.000 (2009-09-07)
	 */
	function CheckBox($name, $w, $checked=false, $prop=array(), $opt=array(), $onvalue='Yes', $x='', $y='', $js=false) {
		if ($x === '') {
			$x = $this->x;
		}
		if ($y === '') {
	 ...
	 * @since 4.8.000 (2009-09-07)
	 */
	function Button($name, $w, $h, $caption, $action, $prop=array(), $opt=array(), $x='', $y='', $js=false) {
		if ($x === '') {
			$x = $this->x;
		}
		if ($y === '') {
	 ...
	 */
	function setSignatureAppearance($x=0, $y=0, $w=0, $h=0, $page=-1) {
	 ...
	 * @public
	 */
	function AliasNbPages($numalias="{nb}") {
	 ...
	 * @public
	 */
	function AliasNumPage($numalias="{pnb}") {
	 ...
	 * @public
	 */
	function colorRegistrationBar($x, $y, $w, $h, $transition=true, $vertical=false, $colors='A,R,G,B,C,M,Y,K') {
		$bars = explode(',', $colors);
	 ...
	 * @public
	 */
	function cropMark($x, $y, $w, $h, $type='A,B,C,D', $color=array(0,0,0)) {
	 ...
	 * @public
	 */
	function registrationMark($x, $y, $r, $double=false, $cola=array(0,0,0), $colb=array(255,255,255)) {
	 ...
	 * @public
	 */
	function CoonsPatchMesh($x, $y, $w, $h, $col1=array(), $col2=array(), $col3=array(), $col4=array(), $coords=array(0.00,0.0,0.33,0.00,0.67,0.00,1.00,0.00,1.00,0.33,1.00,0.67,1.00,1.00,0.67,1.00,0.33,1.00,0.00,1.00,0.00,0.67,0.00,0.33), $coords_min=0, $coords_max=1, $antialias=false) {
		$this->Clip($x, $y, $w, $h);
	 ...
	 * @public
	 */
	function Gradient($type, $coords, $stops, $background=array(), $antialias=false) {
		$n = count($this->gradients) + 1;
		$this->gradients[$n] = array();
	 ...
	 * @public
	 */
	function PieSectorXY($xc, $yc, $rx, $ry, $a, $b, $style='FD', $cw=false, $o=0, $nc=2) {
		if ($this->rtl) {
			$xc = $this->w - $xc;
		}
		$op = $this->getPathPaintOperator($style);
	 ...
	 * @public
	 */
	function write2DBarcode($code, $type, $x='', $y='', $w='', $h='', $style='', $align='', $distort=false) {
		if ($this->empty_string(trim($code))) {
			return;
		}
	 ...
	 * @see setHtmlVSpace()
	 */
	function fixHTMLCode($html, $default_css='', $tagvs='', $tidy_options='') {
	 ...
	 * @protected
	 */
	function extractCSSproperties($cssdata) {
		if (empty($cssdata)) {
			return array();
		}
		// remove comments
	 ...
	 * @since 5.1.000 (2010-05-25)
	 */
	function isValidCSSSelectorForTag($dom, $key, $selector) {
	 ...
	 * @since 5.1.000 (2010-05-25)
	 */
	function getCSSdataArray($dom, $key, $css) {
	 ...
	 */
	function getTagStyleFromCSSarray($css) {
	 ...
	 * @since 5.7.000 (2010-08-02)
	 */
	function getCSSBorderWidth($width) {
		if ($width == 'thin') {
			$width = (2 / $this->k);
		} elseif ($width == 'medium') {
			$width = (4 / $this->k);
	 ...
	 * @since 5.7.000 (2010-08-02)
	 */
	function getCSSBorderDashStyle($style) {
	 ...
	 * @since 5.7.000 (2010-08-02)
	 */
	function getCSSBorderStyle($cssborder) {
	 ...
	 * @since 5.9.000 (2010-10-04)
	 */
	function getCSSPadding($csspadding, $width=0) {
	 ...
	 * @since 5.9.000 (2010-10-04)
	 */
	function getCSSMargin($cssmargin, $width=0) {
	 ...
	 */
	function getCSSBorderMargin($cssbspace, $width=0) {
	 ...
	 */
	function getCSSFontSpacing($spacing, $parent=0) {
	 ...
	 * @since 5.9.000 (2010-10-02)
	 */
	function getCSSFontStretching($stretch, $parent=100) {
...
Putting 1 is equivalent to putting 0 and calling Ln() just after. Default value: 0.
	 ...
	 * @protected
	 */
	function openHTMLTagHandler($dom, $key, $cell) {
		$tag = $dom[$key];
	 ...
	 * @protected
	 */
	function closeHTMLTagHandler($dom, $key, $cell, $maxbottomliney=0) {
		$tag = $dom[$key];
		$parent = $dom[($dom[$key]['parent'])];
	 ...
	 * @protected
	 */
	function addHTMLVertSpace($hbz=0, $hb=0, $cell=false, $firsttag=false, $lasttag=false) {
		if ($firsttag) {
			$this->Ln(0, $cell);
			$this->htmlvspace = 0;
			return;
	 ...
	 * @since 5.7.000 (2010-08-03)
	 */
	function getBorderStartPosition() {
		if ($this->rtl) {
			$xmax = $this->lMargin;
		} else {
	 ...
	 * @since 5.7.000 (2010-08-03)
	 */
	function drawHTMLTagBorder($tag, $xmax) {
	 ...
	 * @since 4.2.010 (2008-11-14)
	 */
	function setGraphicVars($gvars, $extended=false) {
	 ...
	 */
	function addHTMLTOC($page='', $toc_name='TOC', $templates=array(), $correct_align=true, $style='', $color=array(0,0,0)) {
	 ...
	 * @since 4.5.029 (2009-03-19)
	 */
	function rollbackTransaction($self=false) {
		if (isset($this->objcopy)) {
	 ...
	 */
	function revstrpos($haystack, $needle, $offset = 0) {
	 ...
	 * @since 4.9.001 (2010-03-28)
	 */
	function setColumnsArray($columns) {
	 ...
	 */
	function serializeTCPDFtagParameters($pararray) {
	 ...
	 */
	function setTextRenderingMode($stroke=0, $fill=true, $clip=false) {
	 ...
	 * @protected
	 */
	function hyphenateWord($word, $patterns, $dictionary=array(), $leftmin=1, $rightmin=2, $charmin=1, $charmax=8) {
	 ...
	 * @public
	 */
	function getHyphenPatternsFromTEX($file) {
	 ...
	 * @public
	 */
	function hyphenateText($text, $patterns, $dictionary=array(), $leftmin=1, $rightmin=2, $charmin=1, $charmax=8) {
	 ...
	 */
	function setRasterizeVectorImages($mode) {
	 ...
	 * @protected
	 */
	function getPathPaintOperator($style, $default='S') {
		$op = '';
		switch($style) {
			case 'S':
			case 'D': {
	 ...
	 * @since 5.3.002 (2010-06-07)
	 */
	function setFontSubsetting($enable=true) {
	 ...
	 */
	function startTemplate($w=0, $h=0) {
		if ($this->inxobj) {
	 ...
	 */
	function printTemplate($id, $x='', $y='', $w=0, $h=0, $align='', $palign='', $fitonpage=false) {
	 ...
	 * @since 5.9.000 (2010-09-29)
	 */
	function setFontStretching($perc=100) {
	 ...
	 * @since 5.9.000 (2010-09-29)
	 */
	function setFontSpacing($spacing=0) {
	 ...
	 */
	function setPageRegions($regions=array()) {
	 ...
	 * @since 5.9.003 (2010-10-13)
	 */
	function checkPageRegions($h, $x, $y) {
		// set default values
		if ($x === '') {
			$x = $this->x;
		}
	 ...
	 * @protected
	 */
	function getSVGTransformMatrix($attribute) {
		// identity matrix
	 ...
	 * @protected
	 */
	function getTransformationMatrixProduct($ta, $tb) {
		$tm = array();
	 ...
	 * @protected
	 */
	function setSVGStyles($svgstyle, $prevsvgstyle, $x=0, $y=0, $w=1, $h=1, $clip_function='', $clip_params=array()) {
		$objstyle = '';
	 ...
	 * @protected
	 */
	function getVectorsAngle($x1, $y1, $x2, $y2) {
		$dprod = ($x1 * $x2) + ($y1 * $y2);
	 ...
	 * @protected
	 */
	function startSVGElementHandler($parser, $name, $attribs, $ctm=array()) {
	 ...
	 * @protected
	 */
	function endSVGElementHandler($parser, $name) {
		switch($name) {
			case 'defs': {
	 ...
	 * @protected
	 */
	function segSVGContentHandler($parser, $data) {
		$this->svgtext .= $data;
	}
